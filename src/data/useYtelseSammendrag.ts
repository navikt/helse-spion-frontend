      ...
      if (response.status === 401) {
        ...
        return response.json().then((data) => {
          const convertedResponseData =
            convertResponseDataToYtelseSammendrag(data);
...
export const convertResponseDataToYtelseSammendrag = (
  ...
  data.forEach((element) => {
    ...
    const refusjonsdager: any = dayjs(element.periode.tom).diff(
      dayjs(element.periode.fom),
      'day'
    );
    ...
    if (ytelsesElementIndex === -1) {
      ytelsesElement = {
        ...
        navn: (
          element.arbeidsforhold.arbeidstaker.fornavn +
          ' ' +
          element.arbeidsforhold.arbeidstaker.etternavn
        ).trim(),
        antall_refusjoner: 1,
      ...
      ytelsesElement.max_refusjon_dager =
        ytelsesElement.max_refusjon_dager > refusjonsdager
          ? ytelsesElement.max_refusjon_dager
          : refusjonsdager;
    }
  });
